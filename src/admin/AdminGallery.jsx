...
import { IconImage, IconEdit, IconTrash } from '../components/Icons';
          ...
          <div className="panel-body" style={{padding:0}}>
            {items.length===0?<p className="empty-state">No photos yet.</p>:items.map(item=>(
              <div className="item-card" key={item.id}>
                <div className="item-thumb">{item.imageUrl?<img src={item.imageUrl} alt="" style={{width:52,height:52,objectFit:'cover'}}/>:<IconImage />}</div>
                ...
                <div className="item-actions">
                  <button className="btn-icon" onClick={()=>{setEditId(item.id);setForm({imageUrl:item.imageUrl||'',caption:item.caption||'',status:item.status||'visible'});}}><IconEdit /></button>
